...
export class BasketService {
  ...
  private basketSourcce = new BehaviorSubject<IBasket>(null);
  basket$ = this.basketSourcce.asObservable();
  ...
  getBasket(id: string): Observable<void> {
    return this.http.get<IBasket>(this.baseUrl + 'basket?id=' + id)
      .pipe(
        map(basket => {
          this.basketSourcce.next(basket);
  ...
  setBasket(basket: IBasket): any {
    return this.http.post<IBasket>(this.baseUrl + 'basket', basket).subscribe(response => {
      this.basketSourcce.next(response);
      this.calculateTotals();
    }, error => {
      console.error(error);
    });
  ...
  getCurrentBasketValue(): IBasket {
    return this.basketSourcce.value;
  }
